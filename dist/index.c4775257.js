...
const notValidEntry = function() {
    if (day.value > 31 || day.value == "" || day.value <= 0) showMessage(day, "Not a valid Day");
    if (month.value >= 13 || month.value == "" || month.value < 0) showMessage(month, "Not a valid Month");
    if (year.value > now.getFullYear() || year.value == "" || year.value <= 0) showMessage(year, "Not a valid year");
};
const calcAge = function() {
    ...
    else if (now.getFullYear() = year.value) calcedYear.textContent = "00";
    else calcedYear.textContent = "--";
    if (now.getMonth() + 1 >= month.value) calcedMonth.textContent = thisMonth;
    else if (12 < month.value) calcedMonth.textContent = "--";
    else calcedMonth.textContent = 13 - month.value;
    if (now.getDate() >= day.value) calcedDay.textContent = thisDay;
    else if (day.value > 31) calcedDay.textContent = "--";
    else calcedDay.textContent = 30 + thisDay;
    if (now.getDate() <= day.value && now.getMonth() + 1 <= month.value) calcedYear.textContent = +(now.getFullYear() - year.value);
    notValidEntry();
